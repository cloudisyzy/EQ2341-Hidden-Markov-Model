...
class DiscreteD:
    ...
    !!!! It should start from 0 to length(ProbMass) due to index in python starts from 0!!!!
...
#         valueList = np.arange(len(self.probMass))
#         R = np.random.choice(valueList, size=nData, p=self.probMass)
#         return R
    ...
    def rand(self, nData):
        ...
        #*** Insert your own code here and remove the following error message
        valueList = np.arange(1, len(self.probMass) + 1)
        R = np.random.choice(valueList, size=nData, p=self.probMass)
        return R
